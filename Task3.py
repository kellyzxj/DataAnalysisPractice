...
for call in calls:
    if call[0].startswith('(080)'): 
        ...
        if call[1].startswith('(0'):
            landline=call[1].split('(')[1].split(')')[0]
            if landline not in call_no:
                call_no.append(landline)
...
print("%.2f"%(rate*100) + "% percent of calls from fixed lines in Bangalore are calls to other fixed lines in Bangalore.")